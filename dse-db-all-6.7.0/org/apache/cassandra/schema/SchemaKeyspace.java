...
public final class SchemaKeyspace {
    ...
    public static final List<String> ALL = UnmodifiableArrayList.of((new String[]{"columns", "hidden_columns", "dropped_columns", "triggers", "types", "functions", "aggregates", "indexes", "tables", "views", "keyspaces"}));
    ...
    static Mutation.SimpleBuilder makeUpdateTableMutation(final KeyspaceMetadata keyspace, final TableMetadata oldTable, final TableMetadata newTable, final long timestamp) {
        final Mutation.SimpleBuilder builder = makeCreateKeyspaceMutation(keyspace.name, keyspace.params, timestamp);
        addTableToSchemaMutation(newTable, false, builder);
        final MapDifference<ByteBuffer, ColumnMetadata> columnDiff = (MapDifference<ByteBuffer, ColumnMetadata>) Maps.difference((Map) oldTable.columns, (Map) newTable.columns);
        ...
        final MapDifference<ByteBuffer, DroppedColumn> droppedColumnDiff = (MapDifference<ByteBuffer, DroppedColumn>) Maps.difference((Map) oldTable.droppedColumns, (Map) newTable.droppedColumns);
        for (final DroppedColumn column2 : droppedColumnDiff.entriesOnlyOnRight().values()) {
            addDroppedColumnToSchemaMutation(newTable, column2, builder);
        }
        for (final ByteBuffer name2 : droppedColumnDiff.entriesDiffering().keySet()) {
            addDroppedColumnToSchemaMutation(newTable, (DroppedColumn) newTable.droppedColumns.get((Object) name2), builder);
        }
        final MapDifference<String, TriggerMetadata> triggerDiff = triggersDiff(oldTable.triggers, newTable.triggers);
        for (final TriggerMetadata trigger : triggerDiff.entriesOnlyOnLeft().values()) {
            dropTriggerFromSchemaMutation(oldTable, trigger, builder);
        }
        for (final TriggerMetadata trigger : triggerDiff.entriesOnlyOnRight().values()) {
            addTriggerToSchemaMutation(newTable, trigger, builder);
        }
        final MapDifference<String, IndexMetadata> indexesDiff = indexesDiff(oldTable.indexes, newTable.indexes);
        for (final IndexMetadata index : indexesDiff.entriesOnlyOnLeft().values()) {
            dropIndexFromSchemaMutation(oldTable, index, builder);
        }
        for (final IndexMetadata index : indexesDiff.entriesOnlyOnRight().values()) {
            addIndexToSchemaMutation(newTable, index, builder);
        }
        for (final MapDifference.ValueDifference<IndexMetadata> diff : indexesDiff.entriesDiffering().values()) {
    ...
    public static void validateNonCompact() throws StartupException {
        ...
        for (UntypedResultSet.Row row : ((UntypedResultSet) query(query, new Object[0]).blockingGet())) {
    ...
    private static UDFunction createUDFFromRow(UntypedResultSet.Row row, Types types) {
        ...
        for (String language : row.getFrozenList("argument_types", UTF8Type.instance)) {
    ...
    private static Functions fetchUDAs(String keyspaceName, Functions udfs, Types types) {
        ...
        for (UntypedResultSet.Row row : ((UntypedResultSet) query(query, new Object[]{keyspaceName}).blockingGet())) {
    ...
    static Keyspaces fetchKeyspaces(Set<String> toFetch) {
        ...
        for (UntypedResultSet.Row row : ((UntypedResultSet) query(query, new Object[]{new ArrayList(toFetch)}).blockingGet())) {
    ...
    static {
        ...
        ALL_TABLE_METADATA = UnmodifiableArrayList.of((new TableMetadata[]{Keyspaces, Tables, Columns, ViewColumns, Triggers, DroppedColumns, Views, Types, Functions, Aggregates, Indexes}));
    }
