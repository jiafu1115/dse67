...
public class Memtable implements Comparable<Memtable> {
   ...
   private static MemtablePool createMemtableAllocatorPool() {
      ...
      case 1:
         ...
         return new HeapPool(memoryLimit, memtableCleanupThreshold, cleaner);
      case 2:
   ...
   List<Memtable.FlushRunnable> createFlushRunnables(LifecycleTransaction txn, DiskBoundaries diskBoundaries) {
      ...
      if(boundaries == null) {
         return UnmodifiableArrayList.of((Object)(new Memtable.FlushRunnable(this.subranges, (Directories.DataDirectory)null, (PartitionPosition)null, (PartitionPosition)null, txn, null)));
      } else {
   ...
   public void makeUnflushable() throws Exception {
      for(int i = 0; i < this.subranges.length; ++i) {
         ...
         Threads.evaluateOnCore(() -> {
            ...
            return null;
         }, i, TPCTaskType.UNKNOWN).reduceBlocking((Object)null, (a, b) -> {
            return null;
         });
      }
      ...
      private void writeSortedContents() {
         if(!this.state.compareAndSet(Memtable.FlushRunnableWriterState.IDLE, Memtable.FlushRunnableWriterState.RUNNING)) {
            ...
            try {
               ...
               if(!Memtable.this.metadata.partitioner.equals(DatabaseDescriptor.getPartitioner()) && ((List)partitions).size() > 1) {
                  partitions = UnmodifiableArrayList.of((Object)MergeIterator.get((List)partitions, Comparator.comparing(Partition::partitionKey), new Memtable.MergeReducer(null)));
               }

               Iterator var2 = ((List)partitions).iterator();
